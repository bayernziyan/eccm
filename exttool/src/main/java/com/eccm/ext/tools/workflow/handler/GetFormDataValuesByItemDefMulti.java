...
public class GetFormDataValuesByItemDefMulti extends WorkflowActionHandler {
	...
	public void doHandler(WorkflowAction action,Connection conn) {
		...
		try{	
			...
			List<HashMap<String,Object>> map = WorkflowUtil.getFormDataValueListByItemDefList(conn, (List<String>)_list, wfbean.getTemplateId(),wfbean.getFormId(),whereitem, whereworkflow, Integer.parseInt(String.valueOf(_pagestart)), Integer.parseInt(String.valueOf(_pagesize)),ispaging);
