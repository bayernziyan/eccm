 ...
 */
public class GetFormGridDataValuesByItemDefMulti extends WorkflowActionHandler {
	private static final Logger LOG = Logger.getLogger(GetFormGridDataValuesByItemDefMulti.class);
	
	public static final String name = "get_formgrid_data_itemdef_multi";
	 ...
	 */
	public static final String param_out_map = name+"_out_formdata_bydef_hashmap";
	...
	public void doHandler(WorkflowAction action,Connection conn) {
		...
		try{	
			...
			HashMap<String,List<HashMap<String,Object>>> map = WorkflowUtil.getFormGridDataValueByItemDefs(conn, (List<String>)_list, wfbean.getFormId(),wfbean.getFormDataId(), whereitem,  Integer.parseInt(String.valueOf(_pagestart)), Integer.parseInt(String.valueOf(_pagesize)),ispaging);
			if(null != map) setArg(action, param_out_map, map);			
		}catch(Exception e){setException(e);LOG.error(e);}
	}
	public GetFormGridDataValuesByItemDefMulti(){}
	public GetFormGridDataValuesByItemDefMulti(HashMap<String,ArrayList<String>> argRelationShip){
